...
class Result
{
    ...
    {
         // Reverse stacks so we can remove from the top easily
        h1.Reverse();
        h2.Reverse();
        h3.Reverse();
        ...
        // Loop until we find equal height or a stack becomes empty
        while (true)
        {
            ...
            {
                sum2 -= h2[h2.Count - 1];
                h2.RemoveAt(h2.Count - 1);
            }
            else
            {
                sum3 -= h3[h3.Count - 1];
                h3.RemoveAt(h3.Count - 1);
            }
    }
